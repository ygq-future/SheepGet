...
选项 A 把 2.5 秒的预算交给了扩展，桌面端因此有一项对应义务：从收到交接请求到写下响应，中间不得夹进可能长时间不返回的调用。它落成两条具体约束。

1. **队列锁不跨越引擎调用**。文件信息窗口的队列锁只保护队列自身的内存状态与本地的落点元数据；会走网络的引擎调用（探测 URL、建任务、确认预下载）一律在锁外执行——锁内取快照，锁外调用，回锁内按 ID 落地。真实引擎的探测会重试三次、还带一次 HEAD 兜底，HTTP 客户端本身没有超时（`Timeout: 0`），锁被它握上几十秒时，每一次交接都排队等在后面，最终在 2.5 秒处静默失败：扩展把下载还给浏览器，桌面端却已受理，用户得到两份下载。
2. **窗口副作用同样在锁外执行**。Wails 的 `Show` / `Focus` / `Hide` / `Emit` 都会同步派发回主线程并等待执行（`application.InvokeSync` 没有超时），主线程被占住是常态而非异常。队列因此把窗口副作用收集起来、在锁外交给一条独立的串行队列执行；串行是必要的——界面上的当前项必须与队列的 `activeIndex` 对应，先显示后投递的次序不能乱。

由此产生两个可观察的中间状态，二者都只在锁外调用出现后才存在，并且都必须有明确行为：

- **一项正在提交**（提交的引擎调用还在路上）时，取消被明确拒绝并给出原因，而不是把这次取消落到队列里的下一项头上；
- **一项正在启动预下载**（`PreDownloadTaskID` 尚未落地）时，提交会等它落地再走「确认预下载」，避免在同一链接上建出两份任务；若这期间用户取消，刚启动的任务会被收掉，不留下用户从未确认过的下载。
