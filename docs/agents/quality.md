...
- `--ci`：默认门禁加 Wails 本机发布构建。各系统分别运行，不能以交叉编译替代运行验证。
...
CI 使用固定 action 提交、固定工具版本和冻结安装，执行各平台默认阶段、Wails 构建与提交范围校验。Ubuntu 使用 GTK3/WebKitGTK 4.1 和 webkit2_41 标签。required 汇总必须在全部必需矩阵成功时才成功；任何失败、取消或跳过不能视为合并许可。远端 workflow 首次执行和保护规则仍需用户授权提交/推送后验证。
...
macOS/Linux 运行结果及远端强制执行均未观察到，不能标为已通过。未建立历史问题忽略基线，未提交或推送本次修改。
