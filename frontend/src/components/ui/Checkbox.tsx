...
export function Checkbox({
  ...
  return (
    <div
      onClick={handleClick}
      className={`group flex cursor-pointer items-center gap-2.5 select-none ${className}`}
    >
      <button
        type="button"
        id={id}
        ...
        onClick={handleClick}
        className={`flex h-4 w-4 shrink-0 cursor-pointer items-center justify-center rounded-md border transition-all duration-150 group-hover:border-[var(--border-focus)] focus:ring-2 focus:ring-[var(--border-focus)] focus:outline-hidden ${
          checked
