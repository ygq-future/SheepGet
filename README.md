...
```text
build/dist/
├── SheepGet_1.0.1_x64-setup.exe                  # Windows x64 NSIS 安装程序（带向导与快捷方式）
├── SheepGet_1.0.1_windows-x64-portable.zip       # Windows x64 绿色免安装便携包（即拷即用）
├── SheepGet_1.0.1_x64_en-US.msi                  # Windows x64 MSI 企业静默安装包
├── SheepGet_1.0.1_extension-chrome-mv3.zip       # 配套浏览器扩展独立更新包（MV3）
